...
import com.hmkcode.android.image.RealPathUtilTonium;
...
public class AsemanToniumActivity extends QtActivity
{
    private static AsemanToniumActivity instance;
    ...
    public AsemanToniumActivity() {
        AsemanToniumActivity.instance = this;
    }

    public static AsemanToniumActivity getActivityInstance() {
        return AsemanToniumActivity.instance;
    }
    ...
    public boolean startNotification(int id, String title, String body, String iconPath, String icon, String channelId, boolean sound, boolean vibrate)
    {
        ...
        Intent notificationIntent = new Intent(this, AsemanToniumActivity.class);
...
//        w.setStatusBarColor(color);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data)
    {
        if (resultCode == RESULT_OK) {
            if (requestCode == AsemanToniumActivity.SELECT_IMAGE) {
                Uri selectedImageUri = data.getData();
                AsemanToniumJavaLayer.selectImageResult( getPath(selectedImageUri) );
            }
        }
    ...
    public String getPath(Uri uri) {
        try {
            ...
            if (Build.VERSION.SDK_INT < 11)
                    realPath = RealPathUtilTonium.getRealPathFromURI_BelowAPI11(this, uri);
            // SDK >= 11 && SDK < 19
            else if (Build.VERSION.SDK_INT < 19)
                    realPath = RealPathUtilTonium.getRealPathFromURI_API11to18(this, uri);
            // SDK > 19 (Android 4.4)
            else
                realPath = RealPathUtilTonium.getRealPathFromURI_API19(this, uri);
            return realPath;
        } catch(Exception e) {
...
//        setTransparentNavigationBar(true);
        ...
        dview.getRootView().getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener(){
            public void onGlobalLayout(){
                  ...
                  if(heightDifference > screenHeight/4) {
                      AsemanToniumJavaLayer.keyboardVisiblityChanged(heightDifference - keyboardExtraHeight);
                  } else {
                      keyboardExtraHeight = heightDifference;
                      AsemanToniumJavaLayer.keyboardVisiblityChanged(0);
                  }
               }
         });
    ...
    protected void checkIntent(Intent intent)
    {
        ...
        if ("text/plain".equals(type))
            AsemanToniumJavaLayer.sendNote(intent.getStringExtra(Intent.EXTRA_SUBJECT), intent.getStringExtra(Intent.EXTRA_TEXT) );
        else
        if("image/png".equals(type) || "image/jpeg".equals(type))
            AsemanToniumJavaLayer.sendImage( (Uri)intent.getExtras().get(Intent.EXTRA_STREAM) );
    }
    ...
    {
        super.onStart();
        AsemanToniumJavaLayer.activityStarted();
    }
    ...
    {
        super.onRestart();
        AsemanToniumJavaLayer.activityRestarted();
    }
    ...
    {
        super.onResume();
        AsemanToniumJavaLayer.activityResumed();
    }

    @Override
    protected void onPause()
    {
        AsemanToniumJavaLayer.activityPaused();
        super.onPause();
    }
    ...
    {
        AsemanToniumJavaLayer.activityStopped();
        super.onStop();
    }
    ...
    {
        AsemanToniumJavaLayer.activityDestroyed();
        super.onDestroy();
    }
}
