...
class TonToolkitListModel::Private
{
public:
...
TonToolkitListModel::TonToolkitListModel(QObject *parent) :
    TonToolkitAbstractListModel(parent)
{
    p = new Private;
}

int TonToolkitListModel::rowCount(const QModelIndex &parent) const
{
    Q_UNUSED(parent)
    return count();
}

QVariant TonToolkitListModel::data(const QModelIndex &index, int role) const
{
    if(index.row() >= count())
        return QVariant();
...
bool TonToolkitListModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    if(index.row() >= count())
        return false;
...
QHash<qint32, QByteArray> TonToolkitListModel::roleNames() const
{
...
int TonToolkitListModel::count() const
{
    return p->list.count();
}

void TonToolkitListModel::setCachePath(const QString &path)
{
    if(p->cachePath == path)
        return;
...
QString TonToolkitListModel::cachePath() const
{
    return p->cachePath;
}

QVariantList TonToolkitListModel::data() const
{
    QVariantList res;
...
void TonToolkitListModel::setData(const QVariantList &data)
{
    if (data == TonToolkitListModel::data())
        return;
...
void TonToolkitListModel::insert(int row, const QVariantMap &map)
{
...
void TonToolkitListModel::append(const QVariantMap &map)
{
    insert(count(), map);
}

void TonToolkitListModel::remove(int row)
{
    if(p->list.count() <= row || row < 0)
        return;
...
void TonToolkitListModel::move(int from, int to)
{
...
void TonToolkitListModel::clear()
{
    beginResetModel();
    p->list.clear();
    saveCache();
...
void TonToolkitListModel::change(const QVariantList &list)
{
...
void TonToolkitListModel::changed(const QList<QVariantMap> &list)
{
...
void TonToolkitListModel::loadCache()
{
...
void TonToolkitListModel::saveCache()
{
    if(p->cachePath.isEmpty())
        return;
...
TonToolkitListModel::~TonToolkitListModel()
{
    delete p;
}
